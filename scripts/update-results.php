    ...
    */
    ...
    for ( $row = $params[START_ROW]; $row <= $maxRow; $row++ ) {
        ...
        $class          = mysqli_real_escape_string(
            $link,
            parse($sheet->getCellByColumnAndRow($params[CLASS_COLUMN], $row)->getValue())
        );
        //Набранные баллы
        $points         = mysqli_real_escape_string(
            $link,
            parse($sheet->getCellByColumnAndRow($params[POINTS_COLUMN], $row)->getValue())
        );
        //Рейтинг (как я понял, % набранных баллов от общего кол-ва)
        $rating         = mysqli_real_escape_string(
            $link,
            parse($sheet->getCellByColumnAndRow($params[RATING_COLUMN], $row)->getValue())
        );
        //Занятое место
        $place          = mysqli_real_escape_string(
            $link,
            parse($sheet->getCellByColumnAndRow($params[PLACE_COLUMN], $row)->getValue())
        );
        ...
        $sql = mysqli_query(
            $link,
            "INSERT INTO `results`(`forId`,`name`,`competition`,`class`,`points`,`rating`,`place`) "
            . "VALUES($forId, '$name', '$competition','$class','$points','$rating','$place')"
        );
        log::e("INSERT INTO `results`(`forId`,`name`,`competition`,`class`,`points`,`rating`,`place`) "
        . "VALUES($forId, '$name', '$competition','$class','$points','$rating','$place'))");
    ...
    /**
     * Найти в строке цифры
     */
    function parse($s) {
        $rez = (int) preg_replace('/[^0-9]/', '', $s);
        return ($rez == 0 ? "" : $rez);
    }
?>