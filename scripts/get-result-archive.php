...
if ($count >= 50) {
    ...
    if ($hasWinner) {
        copy(ROOT . "/images/diploma/5.jpg",
            ROOT . "/results/$id/Благодарственное письмо учителю за подготовку призера конкурса.jpg");
    }
}
...
function pickInt($string) {
    preg_match_all("/\d/m", $string, $array);
    return intval(implode("", $array[0]));
}