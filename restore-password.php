            ...
            <div class="limit-block">
                <div class="flex-block flex-row flex-top flex-left">
                    <div class="center" style="width: 100%;">
                        <form id="restore-form" class="left" style="display: inline-block; max-width: 500px;">
                            <h2>Восстановление пароля</h2>
                            <?php
                                if (isset($_GET['restorekey'])){
                                    if ($link){
                                        $restore_key = mysqli_real_escape_string($link, $_GET['restorekey']);
                                        $rez = mysqli_query($link, "SELECT `userkey`, `date` FROM `restore-password` WHERE `restorekey`='$restore_key'");
